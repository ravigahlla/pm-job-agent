"""Compile the discover → score → digest pipeline."""
...
from pm_job_agent.agents.generation import make_generation_node
...
def build_core_loop_graph(llm: Optional[LLMClient] = None):
    ...
    graph.add_node("generate", make_generation_node(client))
    ...
    graph.add_edge("digest", "generate")
